...
module.exports = function(baseConfig) {
  ...
  // console.log('chunkConfig: ', chunkConfig)
  if (chunkConfig) {
  ...
  if (Array.isArray(baseConfig.webpack.entry)) {
    baseConfig.webpack.entry.map((page) => {
      const conf = {
        ...
        filename: `${page}.html`,
        title: page,
        reactjs: '../../js/react.js' // TODO
      }
      if (chunkConfig) {
        conf.chunks = [ chunkConfig.name, page]; // TODO
      } else {
        conf.chunks = [ page];
      }
  ...
  return {
    ...
    output: {
      // path: path.join(process.cwd(), process.env.PUBLISH_DIR, baseConfig.path),
