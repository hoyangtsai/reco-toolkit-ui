...
function Server(ctx) {
  ...
  return checkPort(ip, port).then(() => {
    ...
    const serverConfig = webpackMerge(
      webpackBaseConfig(baseConfig, argv), webpackDevServerConfig.call(this, baseConfig));
      ...
      new ProgressBarPlugin({
        ...
        callback: () => {
          ...
          if ((argv.o || argv.open) && !startup) {
            const addr = formatAddress(ip, port, 'list.html');
            open(addr);
          }
          startup = true;
        },
    ...
    switch (err.code) {
      case 'EADDRINUSE':
        this.error(Y()`Port ${port} has been used. Try another port instead.`);
        break;
      default:
        this.error(err);
        break;
  ...
  return new Promise((resolve, reject) => {
    if (port > 65535 || port < 1) {
      return reject(new Error(
        Y()`Invalid port number of ${port}. Try another port number between 1 and 65535.`));
    }
